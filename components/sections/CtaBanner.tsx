...
/** Ниже этой ширины — другая раскладка (кадр один и тот же) */
const WIDE = '(min-width: 768px)'

/**
 * Кадр везде один — cta-banner.jpg. Отличается только раскладка.
 *
 * До 768px элементы идут в поток: заголовок, фотография, кнопка под ней.
 * Раньше кнопка лежала поверх снимка и закрывала гирю.
 * От 768px — широкий баннер как в макете: гиря справа, текст слева.
 *
 * Медиавыражение оставлено ради раскладки: варианты структурно разные
 * (фон через fill против картинки в потоке), и display:none тут не годится —
 * скрытый блок всё равно тянет свою копию фотографии по сети.
 */
            ...
            */
            ...
            <div className="overflow-hidden rounded-block border border-line bg-panel-2">
              ...
              <div className="relative mt-5">
                {/* Натуральная пропорция кадра: ничего не обрезается,
                    гиря видна целиком, кнопка её не перекрывает */}
                <Image
                  src="/images/cta-banner.jpg"
                  alt={ctaBanner.imageAlt}
                  width={1983}
